...
export const sisterConcerns: NavLink[] = [
  ...
  { label: "Alive Event Management", href: "/ecosystem/alive-event-management" },
...
export function Logo({
  ...
  return (
    <span className={cn("inline-flex items-center gap-2.5", className)}>
      ...
      >
        <defs>
          <linearGradient id="as-grad-1" x1="405.93" y1="382.59" x2="107.79" y2="189.92" gradientUnits="userSpaceOnUse">
          ...
          <linearGradient id="as-grad-2" x1="78.22" y1="486.73" x2="425.03" y2="354.9" gradientUnits="userSpaceOnUse">
            <stop offset="0" stopColor="#fff" />
            <stop offset="1" stopColor="#fff" stopOpacity="0" />
          </linearGradient>
          <linearGradient id="as-grad-3" x1="76.29" y1="236.08" x2="243.61" y2="11.97" xlinkHref="#as-grad-2" />
        </defs>
        <path fill="url(#as-grad-1)" d="M48.37,261.63c-37.62,80.41,87,47.59,194.06,46s159.42,45.21,159.42,45.21S476.72,323.81,452,270.4s-117.62-43.3-197.38-51.35-96.79-65.23-96.79-65.23S86,181.22,48.37,261.63" />
        <path fill="#00a76d" d="M452,270.4s32.35,66.81-6,117-168.6,105.37-278.36,123.4-157.11-20.56-166-70.51,20.2-55.5,62.24-51,119.31,8.58,206.52-1.7,213-53.1,181.6-117.17" />
        <path fill="url(#as-grad-2)" opacity="0.5" d="M270.86,391.7c-7.13.83-14.4,1.6-21.63,2.28a982.09,982.09,0,0,1-113.09,3.91c-33.4-.75-35.95,115.92,6.23,112,7.87-.73,16.14-1.81,24.58-3.2,113.2-18.6,239.47-74.39,275.74-121.83,19.78-25.87,19.77-56.73,15.64-79.5a53,53,0,0,1-4.64,9.5c-25.16,41-112.42,68.58-182.83,76.88" />
        <path fill="#00a56d" d="M41.41,285.21s.15-11.42,17.37-55.59C79,177.83,125.69,103,189.67,30.44c74-84,138.25,30.08,151.6,52.11S402.58,172.37,405.7,183c5.71,19.57-102.74-56.27-177-51.85-72.6,4.33-164.23,76.74-187.32,154" />
        <path fill="url(#as-grad-2)" opacity="0.5" d="M279.71,15.68C267.8,7.84,254.61,3.06,240.58,4.37c-16.2,1.52-32.28,11.21-47.79,28.82C134.19,99.68,84.32,175.53,62.65,231.13c-.56,1.43-1.1,2.83-1.62,4.19C88.55,194.14,132.29,158,177,139.67c13-33,41.87-85,102.76-124" />
      </svg>
      {withWordmark ? (
    ...
    <AnimatePresence>
      {open ? (
        <motion.div
          ...
          tabIndex={-1}
          className="bg-pine text-paper fixed inset-0 z-50 flex flex-col px-[var(--gutter)] pt-24 pb-12 outline-none"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          transition={{ duration }}
        >
          <button
            ...
            className="text-paper focus-visible:outline-teal-soft absolute top-6 right-[var(--gutter)] font-mono text-sm focus-visible:outline-2 focus-visible:outline-offset-2"
          >
            Close ✕
          </button>
    ...
    <>
      <header
        className={cn(
          ...
          scrolled
            ? "bg-paper/70 border-line border-b backdrop-blur-md"
            : "border-b border-transparent bg-transparent",
        )}
      >
        <div className="mx-auto flex h-16 w-full max-w-[var(--container-max)] items-center justify-between px-[var(--gutter)]">
          <a href="/" aria-label="Alive Spectra — home" className="rounded-sharp focus-visible:outline-teal focus-visible:outline-2 focus-visible:outline-offset-4">
            <Logo />
          </a>
          ...
          <nav className="hidden items-center gap-7 lg:flex">
            {mainNav.map((item) => (
              <a
                key={item.href}
                href={item.href}
                className="text-ink/80 hover:text-pine font-sans text-sm transition-colors"
              >
                {item.label}
              </a>
            ))}
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="text-ink focus-visible:outline-teal font-mono text-sm focus-visible:outline-2 focus-visible:outline-offset-2 lg:hidden"
            >
              Menu
            </button>
          </div>
...
```tsx
import { footerColumns, legal, sisterConcerns, socials } from "@/data/navigation";
      ...
      <div className="mx-auto w-full max-w-[var(--container-max)] px-[var(--gutter)] py-[var(--space-section)]">
        <div className="grid gap-12 lg:grid-cols-[1.2fr_1fr_1fr_1.4fr]">
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="t-mono text-paper/70 flex flex-col gap-1">
              {contact.phones.map((p) => (
                <a key={p} href={`tel:${p.replace(/[^+\d]/g, "")}`} className="hover:text-paper">
                  {p}
                </a>
              ))}
          ...
          {footerColumns.map((col) => (
            <div key={col.title} className="flex flex-col gap-3">
              ...
              <ul className="flex flex-col gap-2">
                {col.links.map((l) => (
                  <li key={l.href}>
                    <a href={l.href} className="t-body text-paper/80 hover:text-paper">
                      {l.label}
                    </a>
                  </li>
                ))}
          ...
          <div className="flex flex-col gap-3">
            ...
            <ul className="grid grid-cols-2 gap-x-4 gap-y-2">
              {sisterConcerns.map((c) => (
                <li key={c.href}>
                  <a href={c.href} className="text-paper/80 hover:text-paper font-sans text-sm">
                    {c.label}
                  </a>
                </li>
              ))}
        ...
        <div className="border-paper/15 mt-12 flex flex-col gap-4 border-t pt-6 sm:flex-row sm:items-center sm:justify-between">
          <p className="t-mono text-paper/50">
            © {new Date().getFullYear()} {siteConfig.name}. {siteConfig.tagline}.
          </p>
          <div className="flex flex-wrap gap-4">
            {socials.map((s) => (
              <a key={s.label} href={s.href} className="t-mono text-paper/60 hover:text-paper">
                {s.label}
              </a>
            ))}
            {legal.map((l) => (
              <a key={l.href} href={l.href} className="t-mono text-paper/60 hover:text-paper">
                {l.label}
              </a>
            ))}
          </div>
...
const sections = [
  ...
  { eyebrow: "Strategy", title: "Scroll to watch the spine shift its wavelength." },
  { eyebrow: "Capital", title: "The nav turns to glass past the first screen." },
  { eyebrow: "Ecosystem", title: "Shell + spine only — the homepage comes next." },
] as const;
      ...
      {sections.map((s, i) => (
        <Section key={s.eyebrow} tone={i % 2 === 1 ? "pine" : "paper"}>
          <Container className="flex min-h-[80vh] flex-col justify-center gap-4">
            ...
            <h2 className={i % 2 === 1 ? "t-display text-paper" : "t-display text-pine"}>
              {s.title}
            </h2>
          </Container>
        </Section>
